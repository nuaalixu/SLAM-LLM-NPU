...
# def get_wav_info(wav_path, type):
#     """
#     Returns:
...
#     # 多个音频文件路径的列表
#     elif type == "wav_paths":
#         wavs = [(base(p), p) for p in sorted(wav_path)]
#     # 处理一个包含音频文件路径的文本文件
#     elif type == "wav_scp":
#         wavs = [line.strip().split() for line in open(wav_path)]
#     # 处理一个包含音频文件的目录
#     elif type == "wav_dir":
...
class FireRedDatasetWav(Dataset):
    def __init__(self, dataset_config, tokenizer=None, split='train'):
        super().__init__()
        cmvn_path = dataset_config.cmvn_file
        self.feature_extractor = ASRFeatExtractor(cmvn_path)
        ...
        if split == "train":
            ...
            raise ValueError("Invalid split")
        # wavs = get_wav_info(data_path, dataset_config.wav_type)
        with open(data_path) as f:
            for line in f:
                item = {}
                parts = line.strip().split()
                if parts:  # 确保 parts 不为空
                    wav_path = parts[0]  # 提取第一个路径
                    # print(wav_path)
                    base = lambda p: os.path.basename(p).replace(".wav", "")
                    key = base(wav_path)
                    item["key"] = key
                    self.data_list[key] = wav_path
                    if self.get_audio_duration(wav_path) > 30.0:
                        print(f"bro, this wav: {wav_path} is too long!")
                        continue
                    item["target"] = ""
                    self.multitask_task_list.append(item)
                else:
...
def get_speech_dataset(dataset_config, tokenizer, split):
    ...
    return dataset
def get_speech_testdataset(dataset_config, tokenizer, split):
    dataset = FireRedDataset(dataset_config, tokenizer, split)
    return dataset
def get_speech_testwavdataset(dataset_config, tokenizer, split):
    dataset = FireRedDatasetWav(dataset_config, tokenizer, split)
    return dataset